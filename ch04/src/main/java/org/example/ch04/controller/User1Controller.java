...
public class User1Controller {
    ...
    public String log() {
        ...
        /*
        *  로그 레벨
        *  DEBUG - 가장 낮은 단계 레벨, 개발용으로 사용, 시스템(WAS)에서도 DEBUG 로그 출력하기 떄문에 사용자 DEBUG 로그 사용 불편
        *  INFO  - 실질적으로 사용자가 사용하는 로그 레벨
        *  WARN  - 잠재적인 에러가 발생할 로직에 사용하는 로그 레벨
        *  ERROR - 명백한 에러가 발생할 로직에 사용하는 로그 레벨, try-catch 에서 catch 절에 사용
        * */
    ...
    public String modify(String userid, Model model) {
        ...
        model.addAttribute(dto); // 키값 생략하면 해당 객체 소문자로 시작하는 타입명 ex) user1DTO
